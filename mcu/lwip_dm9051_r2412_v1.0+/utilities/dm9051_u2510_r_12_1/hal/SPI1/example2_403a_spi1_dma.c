...
//static __inline void hal_spi_mem_setup(uint8_t *fifo_tx, uint8_t *fifo_rx, uint16_t len)
//{
//	DMA_TX_CHANNEL->maddr = (uint32_t)fifo_tx;
//	DMA_TX_CHANNEL->dtcnt = len;
//	DMA_RX_CHANNEL->maddr = (uint32_t)fifo_rx;
//	DMA_RX_CHANNEL->dtcnt = len;
//}

//static __inline void hal_spi_cmd_setup(uint8_t byte)
//{
//	uint8_t dummy_tx[2];
//	uint8_t dummy_rx[2];
//	dummy_tx[0] = byte;
//	hal_spi_mem_setup(dummy_tx, dummy_rx, 1);
//}

//static __inline void hal_spi_rdreg_setup(uint8_t reg, uint8_t *fifo_rx)
//{
//	uint8_t dummy_tx[2];
//	dummy_tx[0] = reg | OPC_REG_R;
//	hal_spi_mem_setup(dummy_tx, fifo_rx, 2);
//}

//static __inline void hal_spi_wtreg_setup(uint8_t reg, uint8_t val)
//{
//	uint8_t dummy_tx[2];
//	uint8_t dummy_rx[2];
//	dummy_tx[0] = reg | OPC_REG_W;
//	dummy_tx[1] = val;
//	hal_spi_mem_setup(dummy_tx, dummy_rx, 2);
//}
...
#define hal_spi_setcmd_func(tbuf) /* tested if apply this macro definition */ \
	hal_spi_halt_dma_xfer(); \
...
//linking...
//.\objects\template.axf: Error: L6218E: Undefined symbol hal_spi_setcmd_func (referred from example2_403a_spi1_dma.o).
//.\objects\template.axf: Error: L6218E: Undefined symbol hal_spi_mem_setup (referred from example2_403a_spi1_dma.o).
//.\objects\template.axf: Error: L6218E: Undefined symbol hal_spi_rdreg_setup (referred from example2_403a_spi1_dma.o).
//.\objects\template.axf: Error: L6218E: Undefined symbol hal_spi_wtreg_setup (referred from example2_403a_spi1_dma.o).
...
//	hal_spi_halt_dma_xfer();
//	hal_spi_rdreg_setup(reg, fifo_rx);
//	hal_spi_start_rdreg_dma_xfer(); //(reg | OPC_REG_R, &val);
...
//	hal_spi_halt_dma_xfer();
//	hal_spi_wtreg_setup(reg, val);
//	hal_spi_start_wtreg_dma_xfer(); //(reg | OPC_REG_W, val);
...
//	hal_spi_halt_dma_xfer();
//	hal_spi_cmd_setup(DM9051_MRCMD | OPC_REG_R);
//	hal_spi_start_cmd_dma_xfer();

//	hal_spi_halt_dma_xfer();
//	hal_spi_mem_setup(&dummy_buf[1], buf, len);
//	hal_spi_start_rdmem_dma_xfer();
...
//	hal_spi_halt_dma_xfer();
//	hal_spi_cmd_setup(DM9051_MWCMD | OPC_REG_W);
//	hal_spi_start_cmd_dma_xfer();

//	hal_spi_halt_dma_xfer();
//	hal_spi_mem_setup(buf, &dummy_buf[1], len);
//	hal_spi_start_wtmem_dma_xfer(); //(buf, len);
