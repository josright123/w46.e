                  ...
                  const TimeInternal *preciseOriginTimestamp, const TimeInternal *correctionField)
{
	...
	printf("<step1> %s, ptpClock->Tms %d sec %d ns\r\n", TransClockFrom(ptpClock), ptpClock->Tms.seconds, ptpClock->Tms.nanoseconds);
    subTime(&ptpClock->Tms, &ptpClock->Tms, correctionField);
	printf("<step2> %s, ptpClock->Tms %d sec %d ns\r\n", TransClockFrom(ptpClock), ptpClock->Tms.seconds, ptpClock->Tms.nanoseconds);
	...
	printf("<step3> %s, ptpClock->Tms %d sec %d ns\r\n", TransClockFrom(ptpClock), ptpClock->currentDS.offsetFromMaster.seconds, ptpClock->currentDS.offsetFromMaster.nanoseconds);
        break;
...
printf("<step4.filter> %s, ptpClock->Tms %d sec %d ns\r\n", TransClockFrom(ptpClock), ptpClock->currentDS.offsetFromMaster.seconds, ptpClock->currentDS.offsetFromMaster.nanoseconds);

    /* check results */
	printf(" *** servo --- %s, on [%s] want to [PTP_SLAVE] offst %d ns (want < %d)\r\n",
			TransClockFrom(ptpClock),
			TransStateName(ptpClock->portDS.portState),
			abs(ptpClock->currentDS.offsetFromMaster.nanoseconds),
			DEFAULT_CALIBRATED_OFFSET_NS); //fgnthjyt..
...
void updateClock(PtpClock *ptpClock)
{
    ...
    printf("updateClock: offset from master:      %10ds %11dns \r\n",
