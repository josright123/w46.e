...
PtpClock ptpClock_emac;
ForeignMasterRecord ptpForeignRecords[DEFAULT_MAX_FOREIGN_RECORDS];

__IO uint32_t PTPTimer = 0;

int	PTPCLOCK_IS_EMAC(PtpClock *ptpClock)
{
	return (ptpClock == &ptpClock_emac) ? 1 : 0;
}
...
int PTPd_Init(void)
{
    ...
    ret = ptpdStartup(&ptpClock_emac, &rtOpts, ptpForeignRecords);

    if (ret != 0)
        return ret;

	printf("--ptpClock_v51.portDS.delayMechanism %u=\r\n", ptpClock_emac.portDS.delayMechanism);
	printf("--ptpClock_v51.portDS.delayMechanism %s\r\n", TransMechanismName(ptpClock_emac.portDS.delayMechanism));

	printf("Mode: rtOpts(RunTimeOpts)_emac ptp mode: %s ", rtOpts.slaveOnly ? "SLAVE" : "MASTER");
	//ptpClock->portDS.portState
	printf("--ptpClock_emac.portDS.portState %s\r\n", TransStateName(ptpClock_emac.portDS.portState));
...
void ptpd_Periodic_Handle_emac(__IO UInteger32 localtime)
{
    ...
    do
    {
        doState(&ptpClock_emac);
        /* if there are still some packets - run stack again */
    }
    while (netSelect(&ptpClock_emac.netPath, 0) > 0);
}
