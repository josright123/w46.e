...
void initData(PtpClock *ptpClock)
{
    ...
    /* Default data set */
    ptpClock->defaultDS.twoStepFlag = DEFAULT_TWO_STEP_FLAG; //FALSE for 'one-step', TRUE for 'two-steps'
...
printf("'PTP_LISTENING'\r\n");
    ...
    if (ptpClock->defaultDS.clockQuality.clockClass < 128)
    {
        ...
        {
            m1(ptpClock);  /* M1 */
printf("'PTP_MASTER(m1)'\r\n");
            return PTP_MASTER;
        }
        else
        {
            p1(ptpClock);
printf("'PTP_PASSIVE(p1)'\r\n");
            return PTP_PASSIVE;
        }
    }
    else
