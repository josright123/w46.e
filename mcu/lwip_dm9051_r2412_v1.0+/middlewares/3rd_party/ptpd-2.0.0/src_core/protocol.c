...
const char *TransClockFrom(PtpClock *ptpClock) {
	return PTPCLOCK_IS_EMAC(ptpClock) ? "emac" :
		PTPCLOCK_IS_V51(ptpClock) ? "v51" : "unknow";
}
const char *TransStateName(Enumeration8 state) {
	return stateName[state];
}
const char *TransMechanismName(Enumeration8 delayMechanism) {
	return MechName[delayMechanism];
}
...
void toState(PtpClock *ptpClock, UInteger8 state)
{
	...
	do {
		printf(" --- %s, ptp_clock protocol to state= '%s'\r\n",
				TransClockFrom(ptpClock),
				TransStateName(state));
	} while(0);
	...
	printf(" *** --- [state PTP_UNCALIBRATED] timerStart: set timer '%s' to %d\r\n",
        ...
        case E2E:
			...
			do {
			UInteger32 interval_ms = getRand(pow2ms(ptpClock->portDS.logMinDelayReqInterval + 1));

	printf(" *** --- [state PTP_UNCALIBRATED] timerStart: set timer '%s' to %d\r\n",
		"DELAYREQ_INTERVAL_TIMER",
		interval_ms);

            timerStart(DELAYREQ_INTERVAL_TIMER, interval_ms, ptpClock->itimer);
...
void doState(PtpClock *ptpClock)
{
    ...
    case PTP_PASSIVE:
        ...
        if (getFlag(ptpClock->events, STATE_DECISION_EVENT))
        {
			#if 1 //.JJ
			do {
				printf(" --- %s, ptp_clock protocol, Do state= '%s'\r\n",
						TransClockFrom(ptpClock),
						TransStateName(ptpClock->portDS.portState));
			} while(0);
			#endif
	...
	//printf(" *** *** --- %s, on [PTP_UNCALIBRATED] enter\r\n", TransClockFrom(ptpClock));
            if (getFlag(ptpClock->events, MASTER_CLOCK_SELECTED))
            {
	printf(" *** --- %s, on [PTP_UNCALIBRATED] to [on 'MASTER_CLOCK_SELECTED']\r\n", TransClockFrom(ptpClock));
	printf(" *** --- %s, on [PTP_UNCALIBRATED] to [PTP_SLAVE]\r\n", TransClockFrom(ptpClock));
            ...
            {
	printf(" *** --- %s, on [PTP_UNCALIBRATED] to [only 'MASTER_CLOCK_CHANGED']\r\n", TransClockFrom(ptpClock));
                DBG("event MASTER_CLOCK_CHANGED\n");
                clearFlag(ptpClock->events, MASTER_CLOCK_CHANGED);
            }
	//printf(" *** *** --- %s, on [PTP_UNCALIBRATED] break\r\n", TransClockFrom(ptpClock));
...
//            DBGVV0("handle: nothing\n"); //modify 2024-11-20
	...
	//printf("ptpClock->msgTmpHeader.messageType :%d\r\n",ptpClock->msgTmpHeader.messageType);	//modify 2023-11-27
...
#if 0
	if (PTPCLOCK_IS_EMAC(ptpClock))
#endif
		getTime(&internalTime);
#if 0
	else if (PTPCLOCK_IS_V51(ptpClock))
		;
#endif
